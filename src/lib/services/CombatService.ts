/**
 * WWM Ultimate Calculator - CombatService
 * 
 * Service façade orchestrant tous les calculateurs de combat.
 * Fournit une API unifiée pour toutes les opérations de combat, DPS, soins et graduation.
 * 
 * Ce service délègue simplement aux calculateurs appropriés sans logique métier supplémentaire.
 * Il agit comme point d'entrée unique pour les fonctionnalités de calcul de combat.
 * 
 * Usage:
 * ```typescript
 * const service = new CombatService();
 * const damage = service.calculateSkillDamage(attacker, skill, target);
 * const rotation = service.simulateRotation(attacker, skills, target, 60);
 * ```
 * 
 * @module services
 * @version 1.0.0
...
export class CombatService {
   ...
   * @example
   * ```typescript
   * const damage = service.calculateSkillDamage(
   *   attacker,
   *   { name: "Flame Strike", physicalRatio: 1.2, cooldown: 5 },
   *   { shield: 0.2, defense: 500, resistance: 300 }
   * );
   * console.log(damage.totalDamage); // 1234
   * ```
   */
   ...
   * @example
   * ```typescript
   * const expectedDamage = service.calculateExpectedSkillDamage(
   *   attacker, skill, target
   * );
   * console.log(expectedDamage.totalDamage); // 987.5 (moyenne)
   * ```
   */
   ...
   * @example
   * ```typescript
   * const ev = service.calculateExpectedValue(attacker, skill, target);
   * console.log(ev.expectedValue);        // 1234.5
   * console.log(ev.variance);             // 456789.0
   * console.log(ev.distribution.miss);    // 0.05
   * console.log(ev.distribution.critical); // 0.30
   * ```
   */
   ...
   * @example
   * ```typescript
   * const rotation = service.simulateRotation(
   *   attacker,
   *   [skill1, skill2, skill3],
   *   target,
   *   60 // 1 minute
   * );
   * console.log(rotation.dps);           // 12345.67
   * console.log(rotation.totalHits);     // 123
   * console.log(rotation.timeline.length); // 123
   * ```
   */
   ...
   * @example
   * ```typescript
   * const expected = service.simulateExpectedRotation(
   *   attacker, skills, target, 60
   * );
   * console.log(expected.dps); // Toujours le même résultat pour les mêmes inputs
   * ```
   */
   ...
   * @example
   * ```typescript
   * const heal = service.calculateHeal(
   *   { attack: 1000, criticalRate: 0.3, criticalMultiplier: 2.0 },
   *   { healRatio: 1.5 },
   *   0.1 // +10% heal bonus
   * );
   * console.log(heal.baseHeal);  // 1650 = 1000 * 1.5 * 1.1
   * console.log(heal.finalHeal); // 3300 si crit (1650 * 2.0)
   * ```
   */
   ...
   * @example
   * ```typescript
   * const expectedHeal = service.calculateExpectedHeal(
   *   { attack: 1000, criticalRate: 0.3, criticalMultiplier: 2.0 },
   *   { healRatio: 1.5 },
   *   0
   * );
   * // expectedHeal = 1500 * 0.7 + (1500 * 2.0) * 0.3 = 1050 + 900 = 1950
   * console.log(expectedHeal.finalHeal); // 1950
   * ```
   */
   ...
   * @example
   * ```typescript
   * const graduation = service.calculateGraduation(
   *   12345,  // votre DPS
   *   15000   // DPS optimal de référence
   * );
   * console.log(graduation.rating);       // 'A'
   * console.log(graduation.percentage);   // 82.3
   * console.log(graduation.recommendation); // "Bon build, quelques améliorations possibles"
   * ```
   */
