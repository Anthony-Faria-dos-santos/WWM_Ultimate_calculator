/**
 * WWM Ultimate Calculator - ComparisonService
 * 
 * Service de comparaison de builds qui utilise CombatService
 * pour effectuer les calculs et produire des résultats comparatifs.
 * 
 * Ce service permet de:
 * - Comparer deux builds complets (DPS, winner, breakdown)
 * - Calculer les gains marginaux par stat (+100 attack, +0.1 crit damage, etc.)
 * - Trouver la meilleure stat à optimiser pour maximiser le DPS
 * 
 * Utilise toujours simulateExpectedRotation (mode Expected Value) pour garantir
 * des résultats stables et comparables sans variance aléatoire.
 * 
 * Usage:
 * ```typescript
 ...
 * const comparison = service.compareTwoBuilds(buildCrit, buildAff, boss);
 * console.log(`Winner: ${comparison.winner}`);
 * 
 * // Trouver la meilleure stat à optimiser
 * const bestStat = service.findBestStat(stats, skills, boss);
 * console.log(`Optimiser: ${bestStat.statName} (+${bestStat.efficiency} DPS/point)`);
 * ```
 * 
 * @module services
 * @version 1.0.0
 ...
 * @example
 * ```typescript
 * const buildCrit: BuildConfig = {
 *   name: 'Build Critique',
 *   stats: { attack: 3000, critRate: 3500, ... },
 *   skills: [skill1, skill2, skill3],
 *   bonuses: [{ source: 'Set bonus', category: BonusCategory.SetBonus, value: 0.2 }],
 * };
 * ```
 */
export interface BuildConfig {
   ...
   * 
   * @example 'Build Critique', 'Build Affinité'
   */
 ...
 *   dpsGain: 250,
 *   dpsGainPercent: 2.0,
 *   efficiency: 2.5, // 250 DPS / 100 points investis
 * };
 * ```
 */
export interface MarginalGainResult {
   ...
   * 
   * @example 'attack', 'critRate', 'critDamage'
   */
   ...
   * 
   * @example 2.5 (2.5 DPS par point d'attack investi)
   */
...
export class ComparisonService {
   ...
   * ```typescript
   * const service = new ComparisonService();
   * const result = service.compareTwoBuilds(
   *   { name: 'Build Crit', stats: statsCrit, skills: rotation, bonuses: [] },
   *   { name: 'Build Aff', stats: statsAff, skills: rotation, bonuses: [] },
   *   bossTarget
   * );
   * console.log(`Winner: ${result.winner} with ${result.dpsAdvantage} DPS advantage`);
   * ```
   */
   ...
   * @example
   * ```typescript
   * const gains = service.calculateMarginalGains(stats, skills, target);
   * console.log(`Meilleure stat à optimiser : ${gains[0].statName}`);
   * console.log(`Efficacité : ${gains[0].efficiency.toFixed(2)} DPS/point`);
   * ```
   */
   ...
   * const bestStat = service.findBestStat(stats, skills, boss);
   * if (bestStat) {
   *   console.log(`Optimiser: ${bestStat.statName}`);
   *   console.log(`ROI: ${bestStat.efficiency.toFixed(2)} DPS/point`);
   * }
   * ```
   */
  public findBestStat(
   ...
   * 
   * @remarks
   * Si la stat n'existe pas dans CharacterBaseStats, retourne les stats inchangées.
   * 
   * @private
   */
  private modifyStat(
