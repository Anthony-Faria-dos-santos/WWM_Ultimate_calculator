 ...
 * 
 * Calcule le Graduation Rate qui compare votre DPS à un build de référence optimal.
 * Cet indicateur permet d'évaluer la progression et l'optimisation d'un build.
 * 
 * Le Graduation Rate est exprimé en pourcentage :
 * - 100% = DPS identique à la référence
 * - < 100% = DPS inférieur à la référence
 * - > 100% = DPS supérieur à la référence
 * 
 * Échelle d'évaluation qualitative :
 ...
 * 
 * @remarks
 * Ce calculateur est standalone et ne dépend d'aucun autre calculateur.
 * Il produit également des recommandations personnalisées basées sur le rating.
 * 
 * @module calculators/GraduationCalculator
 * @version 1.0.0
 */
 ...
 * Formule principale :
 * ```
 * Graduation Rate = (Your DPS / Reference DPS) × 100
 * ```
 * 
 * @remarks
 * Toutes les méthodes publiques sont pures.
 * Les calculs sont simples et ne nécessitent pas de dépendances externes.
 * 
 * @example
 * ```typescript
 * const calculator = new GraduationCalculator();
 * 
 * // Build en progression
 * const result1 = calculator.calculateGraduation(1850, 2100);
 * console.log(`Graduation: ${result1.graduationRate.toFixed(1)}%`);
 * // → "Graduation: 88.1%"
 * console.log(`Rating: ${result1.rating}`);
 * // → "Rating: competitive"
 * console.log(`Gap: ${result1.dpsGap} DPS manquants`);
 * // → "Gap: 250 DPS manquants"
 * 
 * // Build exceptionnel
 * const result2 = calculator.calculateGraduation(2300, 2100);
 * console.log(`Graduation: ${result2.graduationRate.toFixed(1)}%`);
 * // → "Graduation: 109.5%"
...
export class GraduationCalculator {
   ...
   * @example
   * ```typescript
   * const calc = new GraduationCalculator();
   * 
   * // Cas normal : build competitive
   * const result = calc.calculateGraduation(1850, 2100);
   * console.log(result);
   * // → {
   * //   yourDPS: 1850,
   * //   referenceDPS: 2100,
   ...
   * //   rating: 'competitive',
   * //   dpsGap: 250,
   * //   improvementNeeded: 0.135,
   * //   recommendation: 'Focus sur les gains marginaux (sets, talents, rotation)'
   * // }
   * ```
   * 
   * @example
   * ```typescript
   * // Cas exceptionnel : build supérieur à la référence
   * const result = calc.calculateGraduation(2300, 2100);
   * console.log(result);
   * // → {
   * //   yourDPS: 2300,
   * //   referenceDPS: 2100,
   * //   graduationRate: 109.5,
   * //   rating: 'exceptional',
   * //   dpsGap: -200,
   * //   improvementNeeded: 0,
   * //   recommendation: 'Build exceptionnel ! Référence possible pour les autres joueurs'
   * // }
   * ```
   * 
   * @example
   * ```typescript
   * // Edge case : DPS nul
   * const result = calc.calculateGraduation(0, 2100);
   * console.log(result);
   * // → {
   * //   yourDPS: 0,
   * //   referenceDPS: 2100,
   ...
   * //   dpsGap: 2100,
   * //   improvementNeeded: 0,
   * //   recommendation: 'Revoir l\'équipement de base et les ratios de compétences'
   * // }
   * ```
   */
